...
func main2() {
	...
	router.HandleFunc("/exampleProducts", GetProducts).Methods("GET")
	router.HandleFunc("/exampleProducts/search", SearchProducts).Methods("POST")
	router.HandleFunc("/exampleProducts", CreateProduct).Methods("POST")
...
func SearchProducts(writer http.ResponseWriter, request *http.Request) {
	var product Product
	_ = json.NewDecoder(request.Body).Decode(&product)
	var products []Product
	db.Where("name LIKE ?", "%"+product.Name+"%").Find(&products)
